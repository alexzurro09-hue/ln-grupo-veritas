...
import { getCategory, getSubService } from '@/src/content/services';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const isContratosReclamaciones = category === 'juridico' && service === 'contratos-y-reclamaciones';

  if (isContratosReclamaciones) {
    return {
  ...
  const sub = getSubService(category, service);
  const serviceTitle = sub?.title ?? titleCaseFromSlug(service);

  return {
    title: `Especialistas en ${serviceTitle} | LN Grupo Veritas`,
    description:
      sub?.summary ??
...
export default async function Page({ params }: PageProps) {
  const { category: categoryKey, service: serviceSlug } = await params;
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10 py-10 md:py-12">
          <nav className="text-[10px] uppercase tracking-[0.32em] text-veritas-green/70 font-bold">
            <ol className="flex flex-wrap items-center gap-x-3 gap-y-2">
              ...
              <li className="opacity-60">›</li>
              <li>
                <Link href={`/servicios/${categoryKey}`} className="hover:text-veritas-red transition-colors">
                  {categoryLabel}
                </Link>
              </li>
              <li className="opacity-60">›</li>
